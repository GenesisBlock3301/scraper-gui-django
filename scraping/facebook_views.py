...
def facebook_search(request):
    if request.method == "POST":
        ...
        try:
            ...
            status = True
        return render(request, 'Facebook_GUI.html',{'status':status})
        
    return render(request, 'Facebook_GUI.html')
...
def facebook_counter(request):
    # print("Before",URLS)
    for i in Facebook.objects.all():
        print(type(i))
        URLS.append(str(i))
    print(URLS)
