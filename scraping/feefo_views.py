...
def feefo_search(request):
    if request.method == "POST":
        status = False
        try:
            url = request.POST['url']
            FeeFO.objects.create(url=url)
        except:
            status = True
        return render(request, 'Feefo_GUI.html',{'status':status})
        
    return render(request, 'Feefo_GUI.html')
...
def feefo_counter(request):
    # print("Before",URLS)
    for i in FeeFO.objects.all():
        print(type(i))
        URLS.append(str(i))
    print(URLS)
...
def feefo_clear(request):
    if request.method == "POST":
        FeeFO.objects.all().delete()
        return render(request, 'Feefo_GUI.html')
