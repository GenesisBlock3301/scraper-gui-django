...
def youtube_search(request):
    if request.method == "POST":
        ...
        try:
            ...
            Youtube.objects.create(url=url)
        except:
            status = True
        return render(request,'Youtube_GUI.html',{'status':status})
        
    return render(request, 'Youtube_GUI.html')
...
def youtube_counter(request):
    # print("Before",URLS)
    for i in Youtube.objects.all():
        print(type(i))
        URLS.append(str(i))
    print(URLS)
