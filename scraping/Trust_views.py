...
def trust_search(request):
    if request.method == "POST":
        status = False
        try:
            url = request.POST['url']
            Trust.objects.create(url=url)
        except:
            status = True
        return render(request, 'Trust_GUI.html',{'status':status})
        
    return render(request, 'Trust_GUI.html')
...
def trust_counter(request):
    # print("Before",URLS)
    for i in Trust.objects.all():
        # print(type(i))
        URLS.append(str(i))
    print(URLS)
